...
int main() {
    ...
    void* library = dlopen("../zad1/libaddress_book_shared.so", RTLD_LAZY);
    
    struct AddressBook* (*createAddressBook)(int flag);
    *(void **) (&createAddressBook) = dlsym(library, "createAddressBook");
    
    char** (*createData)(const char* name, const char* surname, const char* birth, const char* email, const char* phone, const char* address);
    *(void **) (&createData) = dlsym(library, "createData");
    
    void (*addPerson)(struct AddressBook* addressBook, char **data);
    *(void **) (&addPerson) = dlsym(library, "addPerson");
    
    //void (*printBook)(struct AddressBook* addressBook);
    //printBook = dlsym(library, "printBook");
    
    void (*sortByKey)(struct AddressBook *addressBook, int key);
    *(void **) (&sortByKey) = dlsym(library, "sortByKey");
    
    struct BTreeNode* (*returnRoot)(AddressBook *addressBook);
    *(void **) (&returnRoot) = dlsym(library, "returnRoot");
    
    void (*deleteBook)(struct AddressBook *addressBook);
    *(void **) (&deleteBook) = dlsym(library, "deleteBook");
    
    char** (*searchEl)(struct AddressBook *addressBook, char *word);
    *(void **) (&searchEl) = dlsym(library, "searchEl");
    
    void (*removeEl)(struct AddressBook *addressBook, char *word);
    *(void **) (&removeEl) = dlsym(library, "removeEl");
